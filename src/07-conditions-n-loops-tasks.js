...
function doRectanglesOverlap(rect1, rect2) {
  return !(rect1.top + rect1.height < rect2.top
    ...
    || rect2.left + rect2.width < rect2.left);
}
...
function isCreditCardNumber(ccn) {
  return (Array.from(String(ccn)).reduceRight((acc, curr, index) => {
    if (Array.from(String(ccn)).length % 2 !== 0 ? !(index % 2 === 0) : (index % 2 === 0)) {
      const doubled = 2 * curr;
...
function getDigitalRoot(num) {
  return num > 9
    ? getDigitalRoot(Array.from(String(num)).reduce((acc, curr) => acc + +curr, 0))
    : num;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.includes(str[i])) {
      ...
      if (bracketsPairs[str[i]] === stack[stack.length - 1]) {
        stack.pop();
      } else {
        return false;
      }
